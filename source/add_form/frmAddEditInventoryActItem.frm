...
Begin VB.Form frmAddEditInventoryActItem 
   BackColor       =   &H80000000&
   ...
   End
End
Attribute VB_Name = "frmAddEditInventoryActItem"
...
Private Sub InitFormLayout()
   ...
   Call InitNormalLabel(lblInvAmount, MapText("ยอดนับ"))
   Call InitNormalLabel(Label1, MapText("กิโลกรัม"))
...
Dim Ini As CInventoryActItem

   If Flag Then
      Call EnableForm(Me, False)
      
      If ShowMode = SHOW_EDIT Then
         Set Ini = TempCollection.Item(ID)
        uctlPartLookup.MyCombo.ListIndex = IDToListIndex(uctlPartLookup.MyCombo, Ini.PART_ITEM_ID)
        txtPlanAmount.Text = Ini.INVENTORY_ACT_AMOUNT 'FormatNumber(Ini.INVENTORY_ACT_AMOUNT, 2)
        txtNote.Text = Ini.NOTE
      End If
   End If
...
Dim RealIndex As Long
   ...
   If Not VerifyCombo(lblPart, uctlPartLookup.MyCombo, False) Then
      Exit Function
   End If
   If Not VerifyTextControl(lblInvAmount, txtPlanAmount, False) Then
      Exit Function
   End If
   ...
   If ShowMode = SHOW_ADD Then
      Set Ini = New CInventoryActItem
   Else
      Set Ini = TempCollection.Item(ID)
   End If
   
   Ini.PART_ITEM_ID = uctlPartLookup.MyCombo.ItemData(Minus2Zero(uctlPartLookup.MyCombo.ListIndex))
   Ini.PART_DESC = uctlPartLookup.MyCombo.Text
   Ini.PART_NO = uctlPartLookup.MyTextBox.Text
   Ini.INVENTORY_ACT_AMOUNT = txtPlanAmount.Text
   Ini.NOTE = txtNote.Text
   If ShowMode = SHOW_ADD Then
      Ini.Flag = "A"
      Call TempCollection.add(Ini)
   Else
      If Ini.Flag <> "A" Then
         Ini.Flag = "E"
      End If
