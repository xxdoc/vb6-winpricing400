...
Begin VB.Form frmAddEditInventoryWhAct 
   ClientHeight    =   8490
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   12705
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         PictureBackgroundStyle=   2
      End
      Begin prjFarmManagement.uctlTextBox txtInventoryWhActDesc 
         Height          =   435
         Left            =   1800
         TabIndex        =   1
         Top             =   1440
         ...
         _ExtentY        =   767
      End
      Begin prjFarmManagement.uctlDate uctlInventoryWhActDate 
         Height          =   405
         Left            =   1800
         TabIndex        =   0
         Top             =   870
      ...
      Begin GridEX20.GridEX GridEX1 
         ...
         IntProp7        =   0
         ColumnsCount    =   2
         Column(1)       =   "frmAddEditInventoryWhAct.frx":0000
         Column(2)       =   "frmAddEditInventoryWhAct.frx":00C8
         FormatStylesCount=   5
         FormatStyle(1)  =   "frmAddEditInventoryWhAct.frx":016C
         FormatStyle(2)  =   "frmAddEditInventoryWhAct.frx":02C8
         FormatStyle(3)  =   "frmAddEditInventoryWhAct.frx":0378
         FormatStyle(4)  =   "frmAddEditInventoryWhAct.frx":042C
         FormatStyle(5)  =   "frmAddEditInventoryWhAct.frx":0504
         ImageCount      =   0
         PrinterProperties=   "frmAddEditInventoryWhAct.frx":05BC
      End
      ...
      Begin Threed.SSCommand cmdPrint 
         ...
         ButtonStyle     =   3
      End
      Begin VB.Label lblInventoryWhActDate 
         Alignment       =   1  'Right Justify
         ...
         Width           =   1305
      End
      Begin VB.Label lblInventoryWhActDesc 
         Alignment       =   1  'Right Justify
   ...
   End
End
Attribute VB_Name = "frmAddEditInventoryWhAct"
...
Dim ItemCount As Long
   ...
   If Flag Then
      ...
      m_InventoryWhAct.INVENTORY_WH_ACT_ID = id
      m_InventoryWhAct.QueryFlag = 1
   ...
   If ItemCount > 0 Then
      Call m_InventoryWhAct.PopulateFromRS(1, m_Rs)

      txtInventoryWhActDesc.Text = m_InventoryWhAct.INVENTORY_WH_ACT_DESC
      uctlInventoryWhActDate.ShowDate = m_InventoryWhAct.INVENTORY_WH_ACT_DATE
   End If
...
'       If Not VerifyAccessRight("INVENTORY_WH_ACTUAL_" & InventoryActArea2Text2(InventoryActArea) & "_EDIT", "แก้ไข" & InventoryActArea2Text(InventoryActArea)) Then
'           Call EnableForm(Me, True)
'           Exit Sub
'        End If
'   End If
   If InventoryWhActArea = 1 Or InventoryWhActArea = 2 Or InventoryWhActArea = 3 Then
      If Not VerifyDate(lblInventoryWhActDate, uctlInventoryWhActDate, False) Then
         Exit Function
      End If
      
      If Not CheckUniqueNs(INVENTORY_WH_ACT_UNIQUE, Trim(DateToStringInt(uctlInventoryWhActDate.ShowDate)), id, Trim(str(InventoryWhActArea))) Then
         glbErrorLog.LocalErrorMsg = MapText("มีข้อมูล") & " " & uctlInventoryWhActDate.ShowDate & " " & MapText("อยู่ในระบบแล้ว")
         glbErrorLog.ShowUserError
         Exit Function
      End If
   End If
   ...
   m_InventoryWhAct.INVENTORY_WH_ACT_ID = id
   m_InventoryWhAct.AddEditMode = ShowMode
   m_InventoryWhAct.INVENTORY_WH_ACT_DATE = uctlInventoryWhActDate.ShowDate
   m_InventoryWhAct.INVENTORY_WH_ACT_DESC = txtInventoryWhActDesc.Text
   m_InventoryWhAct.INVENTORY_WH_ACT_AREA = InventoryWhActArea
   Call EnableForm(Me, False)
   Call glbDaily.StartTransaction
   
   If Not glbInventoryAct.AddEditInventoryAct(m_InventoryWhAct, IsOK, False, glbErrorLog) Then
      glbErrorLog.ShowErrorLog (LOG_FILE_MSGBOX)
      SaveData = False
...
Dim lMenuChosen  As Long
   ...
   If lMenuChosen = 1 Then
      ...
      ShowMode = SHOW_EDIT
      id = m_InventoryWhAct.INVENTORY_WH_ACT_ID
      m_InventoryWhAct.QueryFlag = 1
      QueryData (True)
      m_HasModify = False
...
'   ElseIf InventoryWhActArea = 2 Then
...
'         lMenuChosen = oMenu.Popup("ประมาณการรับเข้าวัตถุดิบรายวันจากซัพพลายเออร์", "ปรับค่าหน้ากระดาษ")
   ...
   If Not Report Is Nothing Then
      Call Report.AddParam(m_InventoryWhAct.INVENTORY_WH_ACT_ID, "INVENTORY_WH_ACT_ID")
       Call Report.AddParam(m_InventoryWhAct.INVENTORY_WH_ACT_AREA, "INVENTORY_WH_ACT_AREA")
      Call Report.AddParam(ReportKey, "REPORT_KEY")
      'INVENTORY_WH_ACT_AREA
   End If
...
'      If InventoryWhActArea = 1 Or InventoryWhActArea = 2 Or (InventoryWhActArea = 3 And ShowMode = SHOW_EDIT) Then
'         uctlInventoryWhActDate.Enable = False
'      ElseIf InventoryWhActArea = 13 And ShowMode = SHOW_ADD Then
'       txtInventoryWhActDesc.Text = "IMPORTED " & DateToStringExtEx3(Now)
'        uctlInventoryWhActDate.Enable = True
'        uctlInventoryWhActDate.ShowDate = Now - 1
'        uctlInventoryWhActDate.SetFocus
'      Else
'         uctlInventoryWhActDate.Enable = False
'         uctlInventoryWhActDate.TabStop = False
'      End If
   End If
End Sub
...
Private Sub InitFormLayout()
   ...
   Call InitNormalLabel(lblInventoryWhActDate, MapText("วันที่นับสต๊อก"))
   
   Call InitNormalLabel(lblInventoryWhActDesc, MapText("รายละเอียด"))
...
Dim RealIndex As Long
   ...
   Values(1) = Iai.INVENTORY_WH_ACT_ITEM_ID
   Values(2) = RealIndex
   Values(3) = Iai.PART_NO
   Values(4) = Iai.PART_DESC
   Values(5) = FormatNumber(Iai.INVENTORY_WH_ACT_AMOUNT, 2)
...
Private Sub txtInventoryActDesc_Change()
   m_HasModify = True
End Sub
Private Sub uctlInventoryWhActDate_HasChange()
   m_HasModify = True
End Sub
...
Private Function GetCollection(Tag As String) As Collection
   If Tag = "INV_MATERIAL" Then
      Set GetCollection = m_InventoryWhAct.CollRawMaterials
   ElseIf Tag = "INV_DRUG" Then
      Set GetCollection = m_InventoryWhAct.CollPhamacyRoom
   ElseIf Tag = "INV_SILO" Then
      Set GetCollection = m_InventoryWhAct.CollSilo
   End If
End Function
Private Sub GetTotalAmount()
Dim II As CInventoryWhActItem
Dim SumAmount As Double
   SumAmount = 0

   For Each II In GetCollection(TabStrip1.SelectedItem.Tag)
      If II.Flag <> "D" Then
         SumAmount = SumAmount + II.INVENTORY_WH_ACT_AMOUNT
      End If
   Next II
