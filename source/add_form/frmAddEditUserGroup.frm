...
Begin VB.Form frmAddEditUserGroup 
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin MSComctlLib.TreeView trvMain 
         ...
         Appearance      =   1
      End
      Begin prjFarmManagement.uctlTextBox txtGroupName 
         Height          =   435
         Left            =   2100
         TabIndex        =   0
         Top             =   990
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         PictureBackgroundStyle=   2
      End
      Begin prjFarmManagement.uctlTextBox txtGroupDesc 
         Height          =   435
         Left            =   2100
         TabIndex        =   2
         Top             =   1440
         ...
         _ExtentY        =   767
      End
      Begin prjFarmManagement.uctlTextBox txtMaxUser 
         Height          =   435
         Left            =   2100
         TabIndex        =   3
         Top             =   1890
...
Dim L As Long
   ...
   For Each O In RightItems
      If O.PARENT_ID = PID Then
         If Level = 0 Then
            Set Node = trvMain.Nodes.add(, tvwFirst, NodeID & O.RIGHT_ID, O.RIGHT_ITEM_DESC & " (" & O.RIGHT_ITEM_NAME & ")", 1)
            Node.Tag = O.RIGHT_ID
         ...
         Else
            ...
            Set Node = trvMain.Nodes.add(N, tvwChild, NewNodeID, O.RIGHT_ITEM_DESC & " (" & O.RIGHT_ITEM_NAME & ")", 1)
            Node.Tag = O.RIGHT_ID
            Node.Checked = FlagToCheck(O.RIGHT_STATUS)
...
Dim ItemCount As Long
   ...
   If Flag Then
      ...
      If Not glbAdmin.QueryUserGroup(m_UserGroup, m_Rs, ItemCount, IsOK, glbErrorLog) Then
   ...
   If ItemCount > 0 Then
      ...
      txtMaxUser.Text = m_UserGroup.MAX_USER
...
Dim IsOK As Boolean
   ...
   If Not VerifyTextControl(lblMaxUser, txtMaxUser, True) Then
      Exit Function
   End If
   If Not CheckUniqueNs(USERGROUP_UNIQUE, txtGroupName.Text, id) Then
      glbErrorLog.LocalErrorMsg = MapText("มีข้อมูล") & " " & txtGroupName.Text & " " & MapText("อยู่ในระบบแล้ว")
      glbErrorLog.ShowUserError
      Exit Function
   End If
   ...
   m_UserGroup.MAX_USER = Val(txtMaxUser.Text)
   m_UserGroup.GROUP_STATUS = Check2Flag(chkEnable.Value)

   Call EnableForm(Me, False)
   If Not glbAdmin.AddEditUserGroup(m_UserGroup, IsOK, True, glbErrorLog) Then
      glbErrorLog.ShowErrorLog (LOG_FILE_MSGBOX)
      SaveData = False
      Call EnableForm(Me, True)
      Exit Function
