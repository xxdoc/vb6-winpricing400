...
'   Ivd.ADJUST_FLAG = "Y"
   ...
   For row = 2 To MaxRow
      ...
      If (Ba.PART_ITEM_ID > 0) And (Ba.LOCATION_ID > 0) Then
         Call Ba.AddEditData
      Else
         glbErrorLog.LocalErrorMsg = "ไม่พบข้อมูลวัตถุดิบ '" & Trim(m_ExcelSheet.Cells(row, 2).Value) & "' คลัง '" & Trim(m_ExcelSheet.Cells(row, 1).Value) & "' ในฐานข้อมูล"
         glbErrorLog.ShowUserError
      End If
      Set Ba = Nothing
   ...
   For row = 1 To MaxRow
      ...
      If Not (Cm Is Nothing) Then
         ...
         If BD.SUPPLIER_ID <= 0 And Not (TempCode = Trim(m_ExcelSheet.Cells(row, 2).Value)) Then
            ...
            glbErrorLog.LocalErrorMsg = "ไม่มีรหัส ซัพพลายเออร์ " & Trim(m_ExcelSheet.Cells(row, 2).Value)
