...
Dim planVersion As Long
   ...
   Pn.PLANNING_DESC = "IMPORTED " & DateToStringExtEx3(Now)
   Pn.PLAN_VERSION = planVersion + 1
   ...
   For row = 4 To MaxRow
      ...
      If Val(Val(m_ExcelSheet.Cells(row, 3).Value)) > 0 Or (Trim(m_ExcelSheet.Cells(row, 3).Value) = "F") Then
...
Dim ToDate As Date
   ...
   If Not m_HasActivate Then
      ...
      ElseIf PlanningArea = 4 Then
         ...
         uctlPlanningFrom.ShowDate = FromDate
         uctlPlanningTo.ShowDate = ToDate
      Else
         uctlPlanningDate.Enable = False
         uctlPlanningDate.TabStop = False
         uctlPlanningFrom.SetFocus
         uctlPlanningFrom.ShowDate = getBeginDay("MON")
         uctlPlanningTo.ShowDate = getBeginDay("MON") + 6
      End If
