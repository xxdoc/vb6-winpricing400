...
Begin VB.Form frmPartItem 
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin Threed.SSCommand cmdOK 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmPartItem.frx":3B9E
         ButtonStyle     =   3
      End
   End
End
...
Dim OKClick As Boolean
   ...
   frmAddEditPartItem.HeaderText = MapText("เพิ่มวัตถุดิบ")
...
Dim OKClick As Boolean
   ...
   frmAddEditPartItem.HeaderText = MapText("แก้ไขวัตถุดิบ")
...
Private Sub cmdImport_Click()
   frmImportPartItem.PartGroupID = PartGroupID
   Load frmImportPartItem
   frmImportPartItem.Show 1
   
   Unload frmImportPartItem
   Set frmImportPartItem = Nothing
End Sub
...
Dim Temp As Long
   ...
   If Flag Then
      ...
      m_PartItem.PIG_FLAG = ""
      m_PartItem.PART_GROUP_ID = PartGroupID
...
Dim fmsTemp As JSFormatStyle
   ...
   Set Col = GridEX1.Columns.add '3
   Col.Width = 3780
   Col.Caption = MapText("ชื่อขาย")

   Set Col = GridEX1.Columns.add '4
   Col.Width = 2100
   Col.Caption = MapText("ประเภทวัตถุดิบ")
   
   If PartGroupID = 19 Then 'ขาย
      Set Col = GridEX1.Columns.add '5
      Col.Width = 1200
      Col.Caption = MapText("น.น./หน่วย")
   Else
      Set Col = GridEX1.Columns.add '5
      Col.Width = 0
      Col.Caption = MapText("น.น./หน่วย")
   ...
   Col.Width = 1300
   Col.Caption = MapText("รูปแบบวัตถุดิบ")
   
   Set Col = GridEX1.Columns.add '6
   Col.Width = 2300
   Col.Caption = MapText("หมายเลขวัตถุดิบ PLC")
   
   Set Col = GridEX1.Columns.add '7
   Col.Width = 2000
   Col.Caption = MapText("คลังหลัก PLC")
   
   Set Col = GridEX1.Columns.add '8
   Col.Width = 2000
   Col.Caption = MapText("หมายเลขวัตถุดิบ LAB")
...
Dim RealIndex As Long
   ...
   Values(5) = m_TempPartItem.BILL_DESC
   Values(6) = m_TempPartItem.PART_TYPE_NAME
   Values(7) = m_TempPartItem.WEIGHT_PER_PACK
   Values(8) = m_TempPartItem.UNIT_NAME
   Values(9) = ConvertTypeProduct(m_TempPartItem.PRODUCT_TYPE_ID)
   Values(10) = m_TempPartItem.NUMBER_PLC_ID
   Values(11) = m_TempPartItem.DEFAULT_LOCATION_NAME
   Values(12) = m_TempPartItem.NUMBER_LAB_ID
   Values(13) = DateToStringExtEx2(m_TempPartItem.CREATE_DATE) & "/" & DateToStringExtEx2(m_TempPartItem.MODIFY_DATE)
     
   Set Temp_LTK = GetObject("CLoginTracking", m_LoginTracking, Trim(m_TempPartItem.CREATE_BY), False)
   If Not Temp_LTK Is Nothing Then
      Values(14) = Temp_LTK.USER_NAME
   Else
      Values(14) = ""
   End If
   
   Set Temp_LTK = GetObject("CLoginTracking", m_LoginTracking, Trim(m_TempPartItem.MODIFY_BY), False)
   If Not Temp_LTK Is Nothing Then
      Values(15) = Temp_LTK.USER_NAME
   Else
      Values(15) = ""
   End If
...
'   Values(2) = m_TempPartItem.PART_NO
'   Values(3) = m_TempPartItem.PART_DESC
'   Values(4) = m_TempPartItem.PART_TYPE_NAME
'   Values(5) = m_TempPartItem.UNIT_NAME
'   Values(6) = m_TempPartItem.NUMBER_PLC_ID
'   Values(7) = m_TempPartItem.DEFAULT_LOCATION_NAME
'   Values(8) = m_TempPartItem.NUMBER_LAB_ID
'   Values(9) = DateToStringExtEx2(m_TempPartItem.CREATE_DATE) & "/" & DateToStringExtEx2(m_TempPartItem.MODIFY_DATE)
